...
export class ChannelTileComponent implements OnDestroy, AfterViewInit {
  ...
  async click(record = false) {
    if (this.isProviderVault()) {
      ...
      if (record) {
        this.toastr.info("Recording is managed by the provider-vault playback layer in this hosted version");
        return;
      }
      this.providerVault.play(this.channel!);
      return;
    }
  ...
  isCustom(): boolean {
    if (this.isProviderVault()) return false;
    return this.memory.CustomSourceIds!.has(this.channel?.source_id!);
  }

  isProviderVault(): boolean {
    return this.providerVault.isProviderVaultChannel(this.channel);
  }
  ...
  async copyURL() {
    try {
      if (this.isProviderVault() && navigator.clipboard) {
        await navigator.clipboard.writeText(this.channel?.url ?? "");
        this.error.success("Copied safe provider-vault stream URL");
        return;
      }
