...
import { ProviderVaultService } from "../provider-vault.service";
...
export class HomeComponent implements AfterViewInit, OnDestroy {
  ...
  showScrollTop = false;
  providerVaultMode = false;
  providerVaultChannels: Channel[] = [];
  ...
  async getSources() {
    if (this.providerVault.isBrowserProviderVaultMode()) {
      try {
        const channels = await this.providerVault.loadChannels();
        if (channels.length > 0) {
          this.initProviderVaultMode(channels);
          await this.load();
          return;
        }
      } catch (e) {
        console.warn("DaveTV provider-vault mode unavailable, falling back to native source setup", e);
      }
    }
  ...
  initProviderVaultMode(channels: Channel[]) {
    this.providerVaultMode = true;
    this.providerVaultChannels = channels;
    this.memory.Sources = new Map(
      this.providerVault.providerSources().map((provider) => [
        provider.sourceId,
        {
    ...
    this.memory.XtreamSourceIds = new Set(this.memory.Sources.keys());
    this.filters = {
      source_ids: Array.from(this.memory.Sources.keys()),
      view_type: ViewMode.All,
      media_types: [MediaType.livestream, MediaType.movie],
      page: 1,
      use_keywords: false,
      sort: SortType.provider,
    };
    ...
    this.chkMovie = true;
    this.toast.info("Loaded Apollo Group TV and XtremeHD from DaveTV provider vault");
  }
  ...
  reset() {
    if (this.providerVaultMode) return;
  ...
  async load(more = false) {
    ...
    if (this.providerVaultMode) {
      const channels = this.providerVault.filterChannels(
        this.providerVaultChannels,
        this.filters,
        this.PAGE_SIZE,
      );
      ...
      this.reachedMax = channels.length >= this.providerVault.filterChannels(
        this.providerVaultChannels,
        { ...this.filters, page: 9999 },
        this.PAGE_SIZE,
      ).length;
      this.loading = false;
      return;
