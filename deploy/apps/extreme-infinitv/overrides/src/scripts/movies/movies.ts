        ...
        openPosterMenu({
          ...
          buildStreamUrl: () => {
            if (isVaultCreds(creds)) {
              const containerExt = (entry as any).container_extension || "mp4"
              return vaultStreamUrl(vaultProviderId(creds), "movie", entry.id, containerExt)
            }
  ...
  if (isVaultCreds(creds)) {
    ...
    try {
      const { data, fromCache, age } = await cachedFetch(
        ...
        "vod",
        VOD_TTL_MS,
        async () => vaultMovieItems(await fetchVaultCatalog(vaultProviderId(creds), {
          liveLimit: 0,
          movieLimit: 1200,
          seriesLimit: 0,
        }))
      ...
      log.error("[xt:movies] vault load threw:", e)
...
;(async () => {
  await initI18n()
  creds = await loadCreds()
  if (isVaultCreds(creds) || (creds.host && creds.user && creds.pass)) {
    loadMovies()
  } else {
    showEmptyState()
  }
