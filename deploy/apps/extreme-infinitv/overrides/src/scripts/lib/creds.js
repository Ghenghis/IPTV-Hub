...
export const DAVETV_VAULT_SCHEME = "davetv-vault://"

export const DAVETV_VAULT_PROVIDERS = [
  { id: "apollo", title: "Apollo Group TV" },
  { id: "xtremehd", title: "XtremeHD" },
]
...
export function isDaveTvVaultEntry(entry) {
  return entry?.type === "davetv-vault" && !!entry.providerId
}

export function isDaveTvVaultCreds(creds) {
  return typeof creds?.host === "string" && creds.host.startsWith(DAVETV_VAULT_SCHEME)
}

export function daveTvProviderFromCreds(creds) {
  if (!isDaveTvVaultCreds(creds)) return ""
  return creds.host.slice(DAVETV_VAULT_SCHEME.length)
}
...
function sanitizeMirrors(mirrors) {
  ...
  for (const mirror of mirrors) {
    ...
    const key = `${serverUrl}\0${username}\0${password}`
...
async function ensureMigrated() {
  ...
  migrationPromise = (async () => {
    ...
    if (existing && Array.isArray(existing.entries)) {
      const seeded = seedDaveTvVaultEntries(existing)
      if (seeded.changed) await writeRaw(seeded.state)
      return seeded.state
    }
    ...
    const seed = entry
      ...
      : { entries: [], selectedId: "" }
    const seeded = seedDaveTvVaultEntries(seed)
    if (entry || seeded.changed) {
      await writeRaw(seeded.state)
      await clearLegacy()
    }
    return seeded.state
  })()
...
function seedDaveTvVaultEntries(state) {
  const entries = Array.isArray(state?.entries) ? [...state.entries] : []
  const byId = new Map(entries.map((entry) => [entry._id, entry]))
  let changed = false

  for (const provider of DAVETV_VAULT_PROVIDERS) {
    const id = `davetv-vault-${provider.id}`
    const next = {
      _id: id,
      type: "davetv-vault",
    ...
    if (!existing || existing.type !== next.type || existing.providerId !== next.providerId || existing.title !== next.title) {
      const index = entries.findIndex((entry) => entry._id === id)
      if (index >= 0) entries[index] = { ...existing, ...next }
      else entries.push(next)
      changed = true
    }
  }

  const selectedExists = entries.some((entry) => entry._id === state?.selectedId)
  const selectedId = selectedExists ? state.selectedId : "davetv-vault-apollo"
  if (selectedId !== state?.selectedId) changed = true

  return { changed, state: { entries, selectedId } }
}
...
export async function loadCreds() {
  ...
  if (!e) return { host: "", port: "", user: "", pass: "", liveContainer: LIVE_CONTAINER_DEFAULT }
  if (isDaveTvVaultEntry(e)) {
    return {
      host: DAVETV_VAULT_SCHEME + e.providerId,
      port: "",
      user: "",
      pass: "",
      providerId: e.providerId,
      type: "davetv-vault",
      liveContainer: LIVE_CONTAINER_DEFAULT,
    }
  }
  if (e.type === "m3u") {
    return { host: e.url || "", port: "", user: "", pass: "", liveContainer: LIVE_CONTAINER_DEFAULT }
  }
  if (e.type === "local-m3u") {
    return { host: LOCAL_M3U_SCHEME + e._id, port: "", user: "", pass: "", liveContainer: LIVE_CONTAINER_DEFAULT }
  }
  const candidates = xtreamCandidatesFor(e)
  const pin = mirrorPin.get(e._id) || 0
  return (
    ...
    candidates[0] || {
      ...
      user: e.username || "",
      pass: e.password || "",
      liveContainer: sanitizeLiveContainer(e.liveContainer),
    }
  )
}
...
export function isLikelyM3USource(host, user, pass) {
  if (typeof host === "string" && host.startsWith(DAVETV_VAULT_SCHEME)) return false
 ...
 *
 * @returns {Promise<{ status: "active"|"expired"|"inactive"|"unavailable", expDate?: number|null, reason?: string, httpStatus?: number, raw?: string, latencyMs?: number }>}
 */
export async function testXtreamConnection({ serverUrl, username, password }) {
  ...
  try {
    ...
    if (info.status !== "Active") return { status: "inactive", expDate, latencyMs }
    if (expDate && expDate < Date.now()) return { status: "expired", expDate, latencyMs }
    return { status: "active", expDate, latencyMs }
  } catch (caughtError) {
