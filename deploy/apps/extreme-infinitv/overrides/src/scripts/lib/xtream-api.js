...
import { vaultApiFetch } from "@/scripts/lib/davetv-vault.js"
...
export async function xtreamApiFetch(action, params = {}, opts = {}) {
  ...
  if (isDaveTvVaultEntry(entry)) {
    return vaultApiFetch(entry.providerId, action, params, opts)
  }
...
export async function resolveStreamUrl(buildUrl) {
  ...
  if (isDaveTvVaultEntry(entry)) {
    return buildUrl({
      host: `davetv-vault://${entry.providerId}`,
      port: "",
      user: "",
      pass: "",
