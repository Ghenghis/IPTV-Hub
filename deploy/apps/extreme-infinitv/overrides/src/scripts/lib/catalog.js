...
import {
  ...
  vaultProviderId,
  vaultSeriesItems,
} from "@/scripts/lib/davetv-vault.js"
...
export async function ensureLive(creds, playlistId, opts = {}) {
  if (isVaultCreds(creds)) {
    const { data } = await cachedFetch(playlistId, "live", CHANNELS_TTL_MS, () => retryWithBackoff(async () => {
      return vaultLiveItems(await fetchVaultCatalog(vaultProviderId(creds), {
        liveLimit: 1800,
        movieLimit: 0,
        seriesLimit: 0,
      }))
...
export async function ensureVod(creds, playlistId, opts = {}) {
  if (isVaultCreds(creds)) {
    const { data } = await cachedFetch(playlistId, "vod", VOD_TTL_MS, () => retryWithBackoff(async () => {
      return vaultMovieItems(await fetchVaultCatalog(vaultProviderId(creds), {
        liveLimit: 0,
        movieLimit: 1200,
        seriesLimit: 0,
      }))
...
export async function ensureSeries(creds, playlistId, opts = {}) {
  if (isVaultCreds(creds)) {
    const { data } = await cachedFetch(playlistId, "series", SERIES_TTL_MS, () => retryWithBackoff(async () => {
      return vaultSeriesItems(await fetchVaultCatalog(vaultProviderId(creds), {
        liveLimit: 0,
        movieLimit: 0,
        seriesLimit: 1200,
      }))
