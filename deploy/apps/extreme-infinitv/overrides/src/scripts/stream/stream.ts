...
function buildDirectLiveUrl(id, c = creds) {
  if (isVaultCreds(c)) return vaultStreamUrl(vaultProviderId(c), "live", id, "m3u8")
...
async function loadChannels() {
  ...
  if (hit) {
    if (m3uHit || hit.data?.some?.((item) => item?.url)) indexDirectUrls(hit.data)
  ...
  if (isVaultCreds(creds)) {
    ...
    try {
      const { data, fromCache, age } = await cachedFetch(
        ...
        "live",
        CHANNELS_TTL_MS,
        async () => vaultLiveItems(await fetchVaultCatalog(vaultProviderId(creds), {
          liveLimit: 1800,
          movieLimit: 0,
          seriesLimit: 0,
        }))
      ...
      log.error("[xt:livetv] vault load threw:", e)
      mountVirtualList([])
