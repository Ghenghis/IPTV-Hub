...
export default function VideoPlayer({
    ...
    // Video setup and HLS. initialTime is captured once per source while callbacks flow through refs.
    useEffect(() => {
        const video = videoRef.current;
        if (!video) return;
        ...
        if (useHlsJs) {
            ...
            hls = new Hls({
                ...
                backBufferLength: isLiveHls ? HLS_LIVE_BUFFER_SECONDS : HLS_VOD_BUFFER_SECONDS,
                maxBufferLength: isLiveHls ? HLS_LIVE_BUFFER_SECONDS : HLS_VOD_BUFFER_SECONDS,
                maxMaxBufferLength: isLiveHls ? HLS_LIVE_BUFFER_SECONDS * 2 : HLS_VOD_BUFFER_SECONDS * 2,
                maxBufferSize: isLiveHls ? HLS_LIVE_MAX_BUFFER_BYTES : HLS_VOD_MAX_BUFFER_BYTES,
                maxBufferHole: 0.5,
                startFragPrefetch: true,
                capLevelToPlayerSize: true,
                abrBandWidthFactor: 0.9,
                abrBandWidthUpFactor: 0.35,
                maxStarvationDelay: isLiveHls ? 8 : 12,
                maxLoadingDelay: isLiveHls ? 10 : 14,
        ...
        const handleLoadedMetadata = () => {
            ...
            if (useHlsJs) {
                // HLS seek + play are driven from MANIFEST_PARSED.
                return;
            }
        ...
        >
            ...
            >
                ...
                <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/95 via-black/80 to-transparent px-4 py-2 backdrop-">
                    {/* Background + buffer bar always shows; position/seek is VOD-only. */}
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {isLive && (
                                <div className="flex items-center gap-2 ml-2">
                                    ...
                                    <span className="text-white text-sm font-semibold uppercase tracking-wide">Live</span>
                                </div>
                            )}
                        </div>
