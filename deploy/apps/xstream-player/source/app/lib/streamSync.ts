...
interface StreamSyncOptions {
    ...
    shouldIncludeItem?: (item: any, type: 'live' | 'movie' | 'series') => boolean;
}
...
export async function streamSyncStreams(options: StreamSyncOptions): Promise<void> {
    const { type, action, credentials, signal, onProgress, mapItem, shouldIncludeItem } = options;
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (parsed.type === 'header') {
                        ...
                        if (shouldIncludeItem && !shouldIncludeItem(item, type)) {
                            continue;
                        }
