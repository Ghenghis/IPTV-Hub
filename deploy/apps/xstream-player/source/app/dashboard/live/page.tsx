...
export default function LiveCategories() {
    ...
    const [sort, setSort] = useSortPreference('cat_live', 'name-asc');
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                // Try cache first
                const cached = await getCachedCategories('live');
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 p-4 md:p-6 lg:p-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                {sortedCategories.map((cat) => (
                    <Link
                        key={cat.category_id}
                        href={`/dashboard/live/${cat.category_id}`}
                        data-focusable="true"
                        tabIndex={0}
                        className="group block bg-[#1f1f1f] hover:bg-[#252525] border border-[#333] hover:border-red-600 rounded-xl p-6 transition-all duration-200 hover:shadow-lg hover:to-red-900/10 cursor-pointer relative overflow-hidden focus:outline-none focus:ring-4 focus:ring-red-600 focus:scale-105 z-10"
                    >
