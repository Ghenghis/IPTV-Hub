    ...
    return (
        <div className="space-y-12 p-4 md:p-6 lg:p-10">
            <div className="flex flex-col gap-2">
                <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">My Favorites</h1>
                <p className="text-gray-400 text-sm md:text-base">Your personalized content collection.</p>
            </div>

            {favorites.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-gray-500 space-y-4">
                    <Heart size={64} className="opacity-20" />
                    <p className="text-xl">You have not added any favorites yet.</p>
                    <Link href="/dashboard/search" className="text-red-500 hover:text-red-400 font-medium">
                        Explore content to add some.
                    </Link>
                </div>
            ) : (
                <>
                    {renderSection('Live TV Channels', liveItems, 'No favorite channels.', <Tv className="text-red-500" />)}
                    {renderSection('Movies', movieItems, 'No favorite movies.', <Film className="text-blue-500" />)}
                    {renderSection('Series', seriesItems, 'No favorite series.', <Layers className="text-purple-500" />)}
                </>
            )}
        </div>
    );
