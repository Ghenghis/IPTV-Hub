...
import { isProviderVaultUrl, passthroughStreamUrl } from '@/services/providerVault';
...
function buildProxiedUrl(raw: string) {
  if (isProviderVaultUrl(raw)) return raw;
...
async function resolveStreamUrl(
  ...
  if (!isPortal && item.cmd) {
    if (isProviderVaultUrl(item.cmd)) return passthroughStreamUrl(item.cmd);
  ...
  if (isProviderVaultUrl(raw)) return passthroughStreamUrl(raw);
...
export function useAppNavigation(
  ...
  const playContinueWatching = useCallback(
    async (item: MediaItem, displayTitle: string) => {
      ...
      const stream = isProviderVaultUrl(freshCmd)
        ? passthroughStreamUrl(freshCmd)
        : { raw: freshCmd, proxied: buildProxiedUrl(freshCmd) };
      setRawStreamUrl(stream.raw);
      setStreamUrl(stream.proxied);
      setResumePlaybackState(
