...
import { getProviderVaultChannels, isProviderVaultChannel } from './services/providerVault';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    getProviderVaultChannels()
      .then((providerChannels) => {
        if (providerChannels.length > 0) {
          setChannels(providerChannels);
          setSelectedChannel(providerChannels[0]);
          setSelectedPlaylist('All Channels');
  ...
  const handleEditChannel = (channel: Channel) => {
    if (isProviderVaultChannel(channel)) {
      addToast({
        type: 'info',
        title: 'Managed by DaveAI',
        message: 'Apollo and XtremeHD provider-vault channels are read-only here.',
        duration: 4000,
      });
      return;
    }
