...
static RESERVED_PORTS: LazyLock<Arc<Mutex<HashSet<u16>>>> =
...
impl PackageManager {
    ...
    const fn install_argv(self) -> (&'static str, &'static [&'static str]) {
        match self {
            Self::Npm => ("npm", &["ci"]),
...
impl WebSource {
    ...
    ///
    /// The reservation lock is held across the `port_is_free` probe on purpose: dropping
    /// it between the check and the insert would let two concurrent callers pick the same
    /// port. The probe is a cheap `TcpListener::bind` that drops immediately on success.
...
#[async_trait]
#[allow(clippy::too_many_lines)] // apply()/plan() are state machines; splitting would add noise
