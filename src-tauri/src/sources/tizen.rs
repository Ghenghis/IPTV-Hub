...
#[async_trait]
#[allow(clippy::too_many_lines)] // plan() is a state machine; splitting would add noise
impl Source for TizenSource {
    ...
    async fn plan(&self, app: &AppEntry, _paths: &AppPaths) -> Result<UpdatePlan, CoreError> {
        ...
        steps.push(PlanStep {
            ...
            detail: Some(cfg.device_serial.as_deref().map_or_else(
                || "sdb install <ipk> (auto-select first paired device)".to_string(),
                |serial| format!("sdb -s {serial} install <ipk>"),
            )),
            tag: PlanTag::TimeEstimate,
        });
