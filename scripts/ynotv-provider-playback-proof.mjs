...
const BASE = process.env.YNOTV_BASE || 'https://ynotv.daveai.tech/';
const COOKIE_FILE =
...
const OUT_DIR =
  process.env.OUT_DIR ||
  'C:/Users/Admin/Downloads/VPS/_visual_artifacts/ynotv-provider-playback-proof-20260527';
...
async function runProvider(browser, provider) {
  ...
  try {
    ...
    const providerCard = page.locator('.vault-card').filter({ hasText: provider.label }).first();
    await providerCard.waitFor({ timeout: 70000 });
    const firstChannel = (await providerCard.innerText()).replace(/\s+/g, ' ').trim();

    await page.screenshot({
      path: path.join(OUT_DIR, `ynotv-${provider.id}-home.png`),
      fullPage: true,
    });

    await providerCard.click({ timeout: 15000 });
    ...
    await page.screenshot({
      path: path.join(OUT_DIR, `ynotv-${provider.id}-player.png`),
      fullPage: true,
    });
    ...
    const result = {
      ...
      seen,
      checks: {
        english: /DaveTV Provider Vault|All|Live|Movies|Safe same-origin stream/i.test(state.body),
        firstChannelCurated: /USA AMC/i.test(firstChannel),
        providerVisible: firstChannel.toLowerCase().includes(provider.label.toLowerCase()),
...
const summary = {
  ...
  results,
  artifacts: {
    apolloHome: path.join(OUT_DIR, 'ynotv-apollo-home.png'),
    apolloPlayer: path.join(OUT_DIR, 'ynotv-apollo-player.png'),
    xtremehdHome: path.join(OUT_DIR, 'ynotv-xtremehd-home.png'),
    xtremehdPlayer: path.join(OUT_DIR, 'ynotv-xtremehd-player.png'),
  },
};
...
assert(summary.ok, 'YnoTV provider playback proof failed');
