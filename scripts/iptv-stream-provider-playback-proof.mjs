...
const BASE = process.env.IPTV_STREAM_BASE || 'https://iptv-stream.daveai.tech/';
const COOKIE_FILE =
...
const OUT_DIR =
  process.env.OUT_DIR ||
  'C:/Users/Admin/Downloads/VPS/_visual_artifacts/iptv-stream-provider-playback-proof-20260527';
...
function providerSource(provider) {
  return {
    ...
    custom: false,
    kind: 'vault',
    description: 'DaveTV server vault',
  };
}
...
async function runProvider(browser, provider) {
  ...
  await context.addInitScript((source) => {
    localStorage.setItem('streamos_sources', JSON.stringify([source]));
    localStorage.setItem('streamos_filters', JSON.stringify({ type: 'all', value: '', search: '' }));
    localStorage.setItem('streamos_autoResume', JSON.stringify(false));
    localStorage.removeItem('streamos_activeChannel');
  ...
  page.on('console', (message) => {
    if (message.type() === 'error') {
      seen.consoleErrors.push(message.text().slice(0, 400));
    }
  });
  ...
  try {
    ...
    await firstCard.waitFor({ timeout: 70000 });
    const firstChannel = (await firstCard.innerText()).replace(/\s+/g, ' ').trim();

    await page.screenshot({
      path: path.join(OUT_DIR, `iptv-stream-${provider.id}-home.png`),
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: path.join(OUT_DIR, `iptv-stream-${provider.id}-player.png`),
      fullPage: true,
    });
    ...
    const stream200 = seen.providerResponses.filter(
      (entry) =>
        entry.status === 200 &&
        (
          entry.url.includes('/api/provider-vault/aac-hls') ||
          entry.url.includes('/api/provider-vault/stream') ||
          entry.url.includes('/api/provider-vault/segment')
        ),
    );
...
const summary = {
  ...
  results,
  artifacts: {
    apolloHome: path.join(OUT_DIR, 'iptv-stream-apollo-home.png'),
    apolloPlayer: path.join(OUT_DIR, 'iptv-stream-apollo-player.png'),
    xtremehdHome: path.join(OUT_DIR, 'iptv-stream-xtremehd-home.png'),
    xtremehdPlayer: path.join(OUT_DIR, 'iptv-stream-xtremehd-player.png'),
  },
};
...
assert(summary.ok, 'IPTV Stream provider playback proof failed');
